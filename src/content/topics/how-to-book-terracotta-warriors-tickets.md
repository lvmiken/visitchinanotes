---
title: "How to Book Terracotta Warriors Tickets Without Stress"
description: "A practical guide to booking Terracotta Warriors tickets, including the official Qin museum channels, passport use for foreign visitors, why online reservation matters, and what to bring on the day."
publishDate: 2026-06-18
updatedDate: 2026-06-18
planningMoment: "book-transport"
whenToRead: "Best read before your Xi'an dates are finalized, especially if the Terracotta Army is one of the main reasons Xi'an is in your route."
affectsCities:
  - "Xi'an"
featuredCitySlugs:
  - "xian"
keyTakeaways:
  - "The official Qin museum guidance says the Terracotta Warriors site uses real-name reservation, and foreign visitors can book with passport information through the official website or official WeChat channels."
  - "If the Terracotta Army is a key Xi'an priority, handle the booking before overpacking the rest of that day."
  - "Bring the original valid ID document used for reservation, because ticket verification is tied to that document."
beforeYouBookChecklist:
  - "Make sure the Xi'an date is stable enough to justify booking a major anchor sight."
  - "Use the exact passport details you plan to carry on the day."
  - "Do not assume that arriving first and sorting tickets later is the safer strategy."
relatedGuideSlugs:
  - "xian-first-time-guide"
  - "xian-2-day-itinerary"
tags:
  - "Xi'an"
  - "Terracotta Warriors"
  - "Reservations"
author: "Editorial Team"
faq:
  - question: "Can foreign visitors book Terracotta Warriors tickets with a passport?"
    answer: "Yes. The Qin museum's official ticket guidance says visitors, including foreign tourists, can make real-name reservations through the official website and official WeChat channels, using passport information."
  - question: "Do I need to reserve Terracotta Warriors tickets in advance?"
    answer: "Yes, that is the safer approach. The official museum guidance says visitors should complete online ticket purchase before coming, and admission is tied to real-name reservation."
  - question: "What do I need to bring to enter the Terracotta Warriors site?"
    answer: "Bring the original valid ID document used for booking. The official museum guidance says ticket verification is done against the original document, including passports."
---

If the Terracotta Army is one of the main reasons Xi'an is in your trip, do not treat the ticket as an afterthought.

This guide uses the Qin museum's official ticket and visitor guidance checked on **June 18, 2026**.

## Who this is for

This page is for travelers trying to answer:

- what is the official booking channel?
- can foreigners reserve with a passport?
- do I really need to book before showing up?
- what should I bring on the day?

If you are still deciding whether the Terracotta Army should control a major part of your Xi'an stay, keep [Terracotta Army for First-Time Visitors: How Much of Your Xi'an Trip It Should Control](/places/terracotta-army/) open too.

## The short answer

Yes, foreign visitors can book Terracotta Warriors tickets using passport information.

The Qin museum's official guidance says:

- the museum uses **real-name reservation**
- visitors including **Hong Kong, Macao, Taiwan, and foreign visitors** can reserve through the **official website** and **official WeChat channels**
- visitors should bring the **original valid ID document** used for booking

If this is one of the anchor sights in your Xi'an route, handle the reservation before you let the rest of that day harden around vague assumptions.

## Step 1: Use the official museum channel

The official museum site is:

- [Qin museum official site](https://www.bmy.com.cn/)

The official site also points to the official online reservation platform:

- [Official online reservation](https://bmy.albatrip.cn/)

The museum's official ticket guidance also says reservations can be made through the official WeChat accounts:

- `秦始皇帝陵博物院`
- `兵马俑票务在线`

If the ticket matters, start with the official channel rather than whichever reseller page appears first.

## Step 2: Use passport details carefully

The museum's official ticket guidance says passport information can be used for booking.

That sounds simple, but this is still one of the easiest places to create avoidable stress.

Be careful about:

- name spelling
- passport number
- carrying the same original passport on the day

The official visitor guidance says ticket verification is done against the **original valid ID document**, and passports are specifically included.

## Step 3: Book before you travel out there

The museum's official visitor guidance says visitors should complete online ticket purchase before coming, partly to avoid transport congestion and partly because the site uses real-name booking and daily capacity control.

The official guide currently says:

- maximum daily capacity is **65,000 visitors**
- once that limit is reached, ticket sales stop

That does not mean every ordinary day is impossible. It does mean you should not build a must-do Xi'an excursion around "we'll sort it out when we get there."

## Step 4: Understand what the ticket includes

The official museum guide says the visit covers both:

- the **Terracotta Warriors museum area**
- the **Qin Shi Huang Mausoleum Archaeological Site Park (Lishan Garden)**

The museum also says:

- each part usually needs about **1.5 hours**
- a **free shuttle** runs between them

That is very useful because it changes how you think about the day. This is usually not a tiny one-hour stop.

## Step 5: Know the opening rhythm

The current official museum guide shows:

- **high season**: March 16 to November 15, **08:30-17:00**, closing time 18:30
- **low season**: November 16 to March 15, **08:30-16:30**, closing time 18:00
- both main areas stop ticket verification at **17:00**

In practice, that means a slow or vague start can damage the visit more than travelers expect.

## What should most readers do?

If the Terracotta Army is one of the main reasons you came to Xi'an:

- use the official booking channel
- reserve with the correct passport details
- carry the original passport
- keep that day lighter than you first think

This is the same planning logic as the Forbidden City in Beijing: one anchor booking often deserves to shape the day around it.

## Common mistakes

- relying on unofficial ticket paths first
- treating the booking like something to solve after arrival
- entering passport details carelessly
- forgetting to bring the original document used for reservation
- underestimating how much of the Xi'an day this excursion can take

## Which page to read next

- read [Terracotta Army for First-Time Visitors: How Much of Your Xi'an Trip It Should Control](/places/terracotta-army/) if you are still deciding how big a role the site should play
- read [How to Plan a Terracotta Army Half-Day From Xi'an Without Wasting the Rest of the Trip](/guides/terracotta-army-half-day-from-xian/) if the excursion day itself still feels too loose
- read [A Practical 2-Day Xi'an Itinerary for First-Time Visitors](/guides/xian-2-day-itinerary/) if you are placing the excursion inside a real short stay
- read [What to Reserve in Advance for China Trips](/topics/attraction-reservations/) if you want the wider booking logic behind anchor sights
